...
- 注意事项:
    - HashMap/HashSet, 是通过hashCode()方法来得到的，如果自定义类重写了hashCode()，那么一定要警惕影响hashCode()方法的成员的修改。比如，当我将一个对象放入集合中后，如果改变了影响hashCode()值的某个属性，那么将不会改变该对象在数组中的位置(因为hash值的在放入时就确定了)
### 51、Collections工具类
...
## 第十二章节: 泛型(Chapter12)
### 52、泛型引入和介绍
- 在集合中，我们创建一个集合对象，集合中默认可以填写任意的Object类的子类，在遍历集合元素，对元素进行操作时，一般都会将取出的Object对象(所有集合元素取出时都是Object类型)向下转型成我们真实存入的类型。此时如果集合的元素出现无法向下转型的情况则程序会报错(比如一个集合中我想存入的都是Dog类对象，但不小心存入了一个Cat类对象，在编译时不会有任何问题，但在运行时对于Cat对象操作时可能就会报错，得到我们不想要的结果)
- 不使用泛型的问题:
    - 1、集合中可能出现不正确的类型对象，但编译器无法发现
    - 2、在遍历时，需要对每个元素进行向下类型转换(因为默认的取出元素是Object)
- 泛型介绍
    - 泛型又称为参数化类型, 用于解决数据类型的安全性问题
    - 在类声明或实例化时，指定好所需要的具体类型即可。例如: ArrayList<类名> 代替 ArrayList的位置即可
    - 在类定义时, 在类名后跟<E>即可表示该类中E会表示你传入的类型。例如:
    ```
    public class Dog<E> {
        // 在内部可以使用E作为一个类型的指代，比如
        E name;  // 定义一个类型为E的引用

        public E fun() { // 定义一个返回类型为E的方法
        }
    }
    ```
    - 你可以在类中的 将E作为某个属性的类型、某个方法的返回类型、或者参数类型。
    - 对于E究竟是什么类型，是在实例化Dog对象时指定. 例如:Dog<int> dog = new Dog<int>()，那么此时类中使用E就是
- 泛型的声明: 在定义类或者接口时，在类名或接口名后跟<T,K,V...>, 泛型不一定只有一个
    - 其中T,K,V不代表这个值，而是表示类型
    - 常见使用:
    ```
    Iterator<Student> iterator = set.iterator() // 从一个set对象中得到迭代器对象
    ```
- 泛型使用细节: 
    - 泛型只能填入引用类型, 对于基本数据类型需要填入其包装类(如<int>会报错无法填入，需要填入<Integer>)
    - 给泛型指定具体类型之后，可以传入该类型或者其子类(向上转型)
    - 推荐使用泛型的方式是在new array<>省略
    ```
    List<Integer> list1 = new ArrayList<>()
    ```
    - 如果我们不给泛型指定具体类型，则默认Object作为泛型类型

### 53、自定义泛型类/接口和泛型方法
- 自定义泛型类/泛型接口
    - 我们在定义类/接口的时候在类名/接口名后加上<E...>即表示你定义了一个泛型叫做E(可以定义不止一个泛型)。
    - 普通成员(非静态属性和非静态方法)可以使用泛型
    - 类的泛型的类型指定，是在创建对象时确定的(如果没有指定则默认Object)
    - 接口的泛型的类型指定，是在**继承接口**或**实现接口**时确定的(如果没有指定则默认Object)
    ```
    interface A<E>

    interface B<T> implements A<String> // 此时需要指定A接口的泛型类型, 当然此时的String也 可以改为T
    ```
    - 静态方法中不能使用泛型(因为静态方法和成员是在类加载时就已经完成，而泛型的指定却是在实例化对象时，因此静态方法/属性是不能使用泛型的)
        - 需要提醒的是: 在接口(interface)中，所有的属性都是public static的, 因此在接口中也无法使用T name的方式定义属性。
    - 泛型无法初始化如下面的例子。(原因是系统无法直到需要开辟多大的内存空间)
    ```
    T[] ts = new T[8] // 其中T表示泛型
    ```
- 自定义泛型方法
    - 我们在定义方法时也可以指定泛型
    - 泛型方法可以在泛型类中定义，也可以在普通类中定义。定义的方式是在修饰词后<T...>
    ```
    class Car {
        public<T,E> void fly(T t, E e){  // 泛型方法
            ....
            可以使用T,E作为数据类型来使用
        }
    }
    ```
    - 泛型方法的泛型是在方法被调用时，根据传入的参数类型确定的。
    - 注意：如果在方法的修饰符后没有<T..>, 那么该方法就只是一个普通方法！但这个方法中可以使用类声明的泛型
    ```
    public void hi(T t) { // 该方法只是一个普通方法，这里的T其实是类定义的泛型，此处只是使用而已
    }
    ```
    - 泛型方法可以使用类定义的泛型，也可以使用自己本身定义的泛型。

### 54、泛型的继承和通配符
- 泛型不具备继承性，即无法在定义泛型时候左右两变的泛型定义类型不一致
```
List<Object> list = new ArrayList<String>(); // 无法通过编译
```
- 为了能让泛型具有类似多态的特性(父类指针可以执行子类对象)，在对象声明时可以使用通配符来解决
    - <? extends A> 上界通配符: 支持A类本身以及A类的子类，规定了泛型的**上限**。在取元素时，只能获取到上界的类型
    - <?> 无界通配符: 等价于<? extends Object>，即可以支持任何类型。在取元素时，只能获取到Object类型
    - <? super A> 下界通配符: 支持A类本身以及A类的父类(不限于直接父类), 规定了泛型的**下限**。在取元素时，只能获取到Object类型
- 常用情景: 在方法定义中使用泛型通配符，可以实现方法的重载(可以放入多种泛型的List), 既可以约束传入的对象，也可以约束传入对象的泛型
```
public static void printList1(List<? extends Animal> list) {
}
```
- 注意：上界通配符 和 无界通配符 由于无法在编译期间确定泛型的实际类型，因此不能向泛型容器(比如List<?>)添加或删除元素(原因是在编译期间无法知道应该分配给元素多少内存)。使用下界通配符的泛型容器，只能向容器中添加下界元素。

### 55、JUnit使用
- 如果一个类中有很多功能需要测试，我们需要将功能都写到main方法中，如果有多个功能进行测试，我们就需要来回注销/切换会很麻烦
